"""Round 7: own generation and correctly aligned, causal post-item features.
...
SEED = 20260910
CONFIG = {
    'schema_version': 'round7-model-v1', 'model': str(MODEL),
...
def runtime_signature():
    ...
    result['code_hashes'] = {p.name: hashlib.sha256(p.read_bytes()).hexdigest()
                             for p in (Path(__file__), ROOT/'src/run7.py')}
...
def parse_items(response, row_id, truncated=False, expected_items=3):
    ...
    if expected_items == 1 and not matches:
        start, end = len(response)-len(response.lstrip()), len(response.rstrip())
        text = response[start:end]
        lexical = [start+k for k, c in enumerate(text) if c.isalnum()]
        ok = bool(lexical) and not truncated
        item = {'item_id': f'{row_id}__1', 'item_index': 1, 'text': text,
                'start': start, 'end': end,
                'last_content_character': lexical[-1] if lexical else None,
                'parse_ok': ok, 'parse_reason': 'single_item_whole_response' if ok else 'empty_or_truncated'}
        return [item], {'numbered_indices': [], 'extra_numbered_items': [],
                        'unparsed_preamble': '', 'single_item_whole_response': True,
                        'all_items_parse_ok': ok}
    out = []
    for i in range(1, expected_items+1):
        ...
        text = response[start:end]
        ordered = indices == sorted(indices) and all(1 <= k <= expected_items for k in indices)
        lexical = [start+k for k, c in enumerate(text) if c.isalnum()]
    ...
    return out, {'numbered_indices': indices, 'extra_numbered_items': [i for i in indices if not 1 <= i <= expected_items],
...
def generate_answer(tok, model, row):
    ...
    items, parser = parse_items(response, row['row_id'], truncated, row.get('expected_items', len(row['questions'])))
    offsets = token_offsets(tok, response_ids, response).tolist()
    result = {key: row[key] for key in ('row_id', 'question_id', 'split', 'condition')}
    result.update({'dataset': row.get('dataset', 'hotpotqa'), 'expected_items': row.get('expected_items', len(row['questions'])), 'prompt_hash': prompt_hash(row), 'user_prompt_sha256': digest(row['prompt']),
...
def surface_features(tok, row, item, token_count):
    ...
    return [len(item['text']), token_count, index,
            float(index == 1), float(index == 2), float(index == 3 and len(row['questions']) == 3),
            ...
            float(len(row['subjects']) > 1 and row['subjects'][1].lower() in evidence.lower())]
