"""Large hidden1024+z matched against frozen R29 base hidden768+z; development only."""
...
import run29 as control_runner
original = control_runner.original
import extract31 as cache

q, core = original.q, original.core
OUT = ROOT / 'results'
OLD = R29 / 'results'
...
def protocol():
    return {'version': 'r31-large1024-semantic-scene-adaptation-v1', 'seed': SEED,
        ...
        'encoder': 'ModernBERT-large QA-only six-epoch completed run: use its one existing selected epoch. No selection using R16. Extraction separately scheduled; no live upstream scores read in design.',
        'input': 'New frozen R31 large cache: mean hidden1024 over each unchanged four-raw-BPE candidate window including punctuation/whitespace, plus maximum semantic logit over its original lexical tokens. No PCA. Raw1025 float32.',
        'control': 'Reuse completed R29 base hidden768+z models, fold scores, choices and OOF metrics under fixed hashes; never refit controls. Large QA-only versus base auxiliary->QA2 also differs in upstream supervision; backbone, feature width and L2 geometry change together.',
        'probe': {'width': 1025, 'C': .01, 'solver': 'liblinear', 'max_iter': 2000,
            ...
            'weights': 'Unchanged R20/R13 group/condition/answer/window weights and fit-only class factors, group reequalization, total3854. Assert identical to corresponding R29 frozen fit.',
        ...
        'limits': 'Backbone, upstream QA-only versus auxiliary->QA supervision, feature width and L2 geometry change together. This is a matched adaptation budget, not an isolated dimension or model-size effect. No causal or universal learnability claim from this one comparison.',
...
def design():
    ...
    toy = rng.normal(size=(40, 1025)).astype(np.float32)
    ...
    # Hidden pools all raw positions, while z excludes punctuation-only slots.
    toy_h = np.asarray([[2., 4.], [100., 200.], [6., 8.]], np.float32)
    toy_z = np.asarray([-3., 99., 2.], np.float32)
    toy_rows = [{'response_id': 'toy'}]
    toy_windows = [{'row_id': 'toy', 'raw_token_indices': [0, 1, 2], 'lexical_token_indices': [0, 2]}]
    hh, zz = window_features(toy_h, toy_z, toy_rows, np.asarray([[0, 3]]), toy_windows)
    assert np.array_equal(hh, toy_h.mean(axis=0, dtype=np.float64).astype(np.float32)[None, :])
    assert np.array_equal(zz, np.asarray([2.], np.float32))
    # Separate R31 namespace; old encoder globals still point to the base model.
    assert cache.base.MODEL != cache.transfer.base.MODEL
    assert cache.transfer.base.MODEL.name == 'ModernBERT-base'
    cache.freeze(OUT / 'CPU_SELFCHECK.json', {'passed': True, 'group_counts': stats,
        ...
        'alpha_zero_exact_identity': True, 'raw_design_width': 1025, 'control_complete_sha256': control['complete_sha256'],
        'control_models_refitted': 0,
        'all_raw_hidden_mean_lexical_only_logit_max_toy_passed': True,
        'old_encoder_module_globals_unchanged': True,
    ...
    print('R31_DESIGN_CPU_CHECK_PASSED', flush=True)


def window_features(h, z, rows, bounds, windows):
    row_bounds = {row['response_id']: bound for row, bound in zip(rows, bounds)}
    hh = np.empty((len(windows), h.shape[1]), np.float32)
    zz = np.empty(len(windows), np.float32)
    for i, w in enumerate(windows):
        lo, hi = row_bounds[w['row_id']]
        raw = lo + np.asarray(w['raw_token_indices'], int)
        lexical = lo + np.asarray(w['lexical_token_indices'], int)
        assert len(raw) and len(lexical) and (raw >= lo).all() and (raw < hi).all()
        assert (lexical >= lo).all() and (lexical < hi).all()
        hh[i] = h[raw].mean(axis=0, dtype=np.float64).astype(np.float32)
        zz[i] = z[lexical].max()
    return hh, zz


def source_hashes():
    names = [Path(__file__), Path(original.__file__), Path(control_runner.__file__), Path(cache.__file__), Path(core.__file__),
    ...
    names += [cache.OUT / 'complete.json', cache.OUT / 'preparation_complete.json', cache.OUT / 'selected_binding.json']
    return {str(p.resolve()): q.sha(p) for p in names}


def prepare():
    if not (cache.OUT / 'complete.json').exists():
        print('WAIT_FROZEN_LARGE_CACHE_NO_FIT', flush=True); return
    assert not (OUT / 'preparation_complete.json').exists()
    design(); cache.check_complete(); control = verify_control(); snap = source_hashes()
    windows, answers, _, _ = metadata()
    rows = q.lines(SCENE / 'inputs.jsonl')
    bounds = np.load(cache.OUT / 'bounds.npy')
    h = np.load(cache.OUT / 'hidden.npy', mmap_mode='r')
    z = np.load(cache.OUT / 'logit.npy', mmap_mode='r')
    assert h.shape == (14968, 1024) and z.shape == (14968,)
    assert h.dtype == z.dtype == np.float32 and np.isfinite(h).all() and np.isfinite(z).all()
    hh, zz = window_features(h, z, rows, bounds, windows)
    np.savez_compressed(OUT / 'designs.npz', hidden=hh, logit=zz)
    q.save(OUT / 'R29_CONTROL.json', control)
    assert snap == source_hashes()
    q.save(OUT / 'preparation_complete.json', {'status': 'prepared_not_fitted', 'source_sha256': snap,
        'designs_sha256': q.sha(OUT / 'designs.npz'), 'control_sha256': q.sha(OUT / 'R29_CONTROL.json'),
        'windows': 12222, 'raw_width': 1025, 'human_gold': False, 'GPU_used': False,
        'original_validation_test_opened': False})
    print('R31_LARGE_HIDDEN_DESIGNS_PREPARED', (12222, 1025), flush=True)


def check_prepared():
    cache.check_complete()
    p = q.read(OUT / 'preparation_complete.json')
    assert p['source_sha256'] == source_hashes()
    assert p['designs_sha256'] == q.sha(OUT / 'designs.npz')
    assert p['control_sha256'] == q.sha(OUT / 'R29_CONTROL.json')
    return p
...
def fit():
    ...
    with threadpool_limits(limits=4):
        for fold, groups in enumerate(folds):
            ...
            assert model['width'] == 1025
            control = pickle.loads((OLD / f'fold_{fold}_frozen.pkl').read_bytes())
            assert control['groups'] == groups and control['model']['width'] == 769
            ...
            frozen = {'groups': groups, 'PCA': None, 'model': model, 'thresholds': ts,
                ...
                'R29_control_model_sha256': q.sha(OLD / f'fold_{fold}_frozen.pkl')}
            ...
            q.save(OUT / f'fold_{fold}_calibration.json', {k: frozen[k] for k in
                ('groups', 'thresholds', 'alpha_tables', 'selected_alpha', 'human_gold', 'R29_control_model_sha256')})
            names += [f'fold_{fold}_{suffix}' for suffix in ('frozen.pkl', 'scores.npz', 'calibration.json')]
            print('R31_FOLD_FROZEN', fold, 'LRiterations', model['model'].n_iter_.tolist(),
...
def evaluate():
    ...
    spec = importlib.util.spec_from_file_location('_r31_same_evaluation', original.__file__)
    module = importlib.util.module_from_spec(spec); spec.loader.exec_module(module)
    module.OUT = OUT
    module.source_hashes = source_hashes
    module.evaluate()
    old = q.read(OUT / 'R29_CONTROL.json')['methods']
    new = q.read(OUT / 'summary.json')['methods']
    comparisons = {name: {unit: {'R29_base768': old[name][unit]['f1'], 'R31_large1024': new[name][unit]['f1'],
        'difference': new[name][unit]['f1']-old[name][unit]['f1']} for unit in ('windows', 'answers')}
        for name in METHODS}
    q.save(OUT / 'R29_COMPARISON.json', {'methods': comparisons,
        'R29_control_refits': 0, 'control_manifest_sha256': q.sha(OUT / 'R29_CONTROL.json'),
        'human_gold': False, 'source': 'Repeated R16 train only; not public QA.',
        'limits': protocol()['limits']})
    print('R31_LARGE_VS_FROZEN_BASE_COMPARISON_COMPLETE', flush=True)
