...
def main():
    ...
    assert len(retained) == 7482 and len(quarantine) == 1 and not duplicates
    ...
    for index, checked in census.items():
        ...
        assert original['prompt'].startswith(row['released_prompt'] + answer)
        assert original['prompt'][len(row['released_prompt'] + answer):] in ('', '\n[Edited] ')
    ...
    for key, members in reference_members.items():
        if not references[key].strip():
            continue
    ...
    assert len(matches) == 1 and direct == {3705}
    ...
    assert len(group_sizes) == 7408 and max(group_sizes.values()) == 7
    for name, expected in manifest['artifacts_sha256'].items():
    ...
    result = {'passed': True, 'answers': len(retained), 'factual_spans': sum(span_types.values()),
              ...
              'restored_raw_index': 11525, 'blank_reference_not_material_identity': True,
