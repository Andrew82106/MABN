...
OUT=HERE/'llama_features_v3';PLANS=HERE/'data/new_token_plans.jsonl'
VERSION='qa-expanded-fit-uniform-llama-replay-v3-zero-boundary-gate';EXPECTED=3046
...
def protocol():
    return {'version':VERSION,'scope':'Only the frozen3046 additional fit responses; source634/group615 unchanged. Old634 fit and159 calibration features reused read-only; no test data.',
     ...
     'gpu_gate':'Two fixed old-fit engineering rows selected by total input length: base LB/NLL/hidden and five LB definitions must equal old caches exactly. Two new source-length extremes plus one actual raw-first-offset=0 response (deduplicated) repeated exactly; no tolerance relaxation. Engineering rows are not appended to new data.',
    ...
    try:
        ...
        if not all(existing):
            ...
            if gate.exists():assert base.read(gate)['passed'] and base.read(gate)['signature_sha256']==base.digest(sig)
                ...
                zero=[i for i,p in enumerate(plans) if p['original']['response_token_offsets_raw'][0][0]==0]
                assert len(zero)==2
                boundary=min(zero,key=lambda i:plans[i]['response_id'])
                chosen=list(dict.fromkeys([order[0],order[-1],boundary]))
                assert any(plans[i]['original']['response_token_offsets_raw'][0][0]==0 for i in chosen)
                for i in chosen:
                    a=extract(model,plans[i],layouts[i]);b=extract(model,plans[i],layouts[i])
                    assert all(np.array_equal(a[k],b[k]) for k in a)
                    existing[i]=commit(a,plans[i],layouts[i],sig);checks.append({'response_id':plans[i]['response_id'],'kind':'new_repeat','raw_first_offset':plans[i]['original']['response_token_offsets_raw'][0][0],'all_fields_exact':True})
