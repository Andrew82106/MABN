"""Independent CPU audit of atomic-microclaim-relation-expanded-v4.
...
def audit():
    ...
    for answer in answers:
        ...
        for window_index, start in enumerate(expected_eligible, left):
            expected_claims = sorted({answer["example_start"] + claim_id
                                      for token_index in range(start, min(answer["token_count"], start + WINDOW_K))
    ...
    failed_audits = sorted(OUT.glob("FAILURE_independent_audit_*.json"))
    assert len(failed_audits) == 1 and (OUT / "AUDIT_REPAIR.md").exists()
    save_new(OUT / "complete.json", {
        ...
        "superseded_audit_failure_sha256": sha(failed_audits[0]),
        "audit_repair_note_sha256": sha(OUT / "AUDIT_REPAIR.md"),
