"""CERP-v2.1 label-blind, CPU-only structural candidate gate.
...
QC_SALT = "cerp-v2.1-structure-qc-20260913"
...
def valid_may_context(text: str, start: int, end: int) -> bool:
    """Keep calendar May; reject the modal verb may."""
...
def validate_aligned(row: dict) -> tuple[dict | None, str]:
    ...
    edit_start, edit_end = int(row["local_edit_start"]), int(row["local_edit_end"])
    if any(match.start() < edit_end and match.end() > edit_start for match in CITATION_RE.finditer(row["claim_text"])):
        return None, "edit_inside_citation"
    ...
    if kind is None:
        return None, subtype
    parent_type = row["candidate_type"]
    allowed_by_parent = {
        "aligned_relation": {"aligned_relation", "number", "temporal", "negation_direction", "entity"},
        "number": {"number", "temporal"},
        "temporal": {"temporal"},
        "negation_direction": {"negation_direction"},
        "entity": {"entity"},
    }
    if kind not in allowed_by_parent.get(parent_type, set()):
        return None, "parent_type_constraint"
    if kind == "temporal":
        if row["original"].lower() == "may" and not valid_may_context(row["claim_text"], edit_start, edit_end):
            return None, "modal_may_rejected"
        if row["replacement"].lower() == "may" and not valid_may_context(
            row["source_sentence"], int(block["source_edit_start"]), int(block["source_edit_end"])
        ):
            return None, "modal_may_rejected"
    result = dict(row)
    result.update(block)
    result.update({
        ...
        "parent_candidate_type": parent_type,
