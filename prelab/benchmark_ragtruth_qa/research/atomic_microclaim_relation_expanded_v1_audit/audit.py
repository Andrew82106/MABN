"""Independent CPU audit of atomic-microclaim-relation-expanded-v1.
...
def audit():
    ...
    for answer in answers:
        ...
        for start in range(answer["token_count"] - WINDOW_K + 1):
        ...
        for claim_id, example in enumerate(local):
            mapped = sorted({start for token_index in example["lexical_bpe_indices"]
                             for start in range(max(0, token_index - WINDOW_K + 1),
                                                min(token_index, answer["token_count"] - WINDOW_K) + 1)
                             if start in eligible_set})
