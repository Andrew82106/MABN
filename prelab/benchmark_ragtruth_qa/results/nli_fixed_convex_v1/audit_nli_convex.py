"""Independent arithmetic audit of the fixed36 NLI convex candidates."""
...
QA = OUT.parents[1]
UPSTREAM = QA/'results/full_context_nli_initialization_v2/full_finetune'
...
def run():
    ...
    assert done['target'] == summary['target'] == 'nli' and done['candidate_count'] == 36
    ...
    assert target['epoch'] == started['selected_epoch'] == 5
    assert target == max(upstream['all_epochs'][1:],key=lambda e:e['selection_key'])
    assert target == summary['target_selected']
    assert started['upstream_complete_sha256'] == sha(UPSTREAM/'complete.json')
    path = UPSTREAM/'epoch_05_token_predictions.npz'
    ...
    for family,control,cpath in controls:
        ...
        for alpha,e in zip(ALPHA,entries):
            assert e['target_epoch'] == 5 and e['target'] == 'nli'
    ...
    report = {'status':'passed','candidate_count':36,'endpoint_count':12,'frozen_upstream_epoch':5,
    ...
    text += ['', '当前公共QA最佳仍为既有large组合0.690281/0.891089。NLI仅为既有校准上的初始化探索，不证明上游数据完全独立，不新增测试或显著性声明。']
    (OUT/'INDEPENDENT_AUDIT.md').write_text('\n'.join(text)+'\n',encoding='utf-8')
    print('NLI_FIXED36_INDEPENDENT_AUDIT_PASSED',flush=True)
