...
OUT = q.ROOT / 'results/large_matched_combination_v2'
LARGE = q.ROOT / 'results/full_context_encoder_large_v1/full_finetune'


def protocol():
    return {
        'version': 'large-matched-combination-v2-convergence-guard', 'peers': list(inputs.PEERS),
...
def run():
    ...
    def save_candidate(name, peer, mode, c, model, scaler, x):
        ...
        entry = {'candidate': name, 'peer': peer, 'mode': mode, 'C': c,
            'input_width': x.shape[1], 'iterations': np.asarray(model.n_iter_).tolist(), 'thresholds': thresholds, 'metrics': q.metrics(meta, score, thresholds),
