...
ROOT = HERE.parent
OUT = ROOT / "results/atomic_microclaim_relation_expanded_v1"
...
AUDIT_CODE = ROOT / "research/atomic_microclaim_relation_expanded_v1_audit/audit.py"
...
def protocol():
    return {
        "version": "atomic-microclaim-relation-expanded-v1",
...
def hypothesis_for(claim):
    ...
    if int(claim["child_index"]) > 0 and antecedent:
        ...
        if subject and subject.casefold() not in core.casefold():
            core = f"{subject} {core}"
    return core
    ...
    with examples_pending.open("w", encoding="utf-8", newline="\n") as example_handle, \
        ...
        for response_index, (partition, source) in enumerate(all_answer_rows):
            ...
            for local_id, (packed, pair_length, premise_length, hypothesis_length) in enumerate(zip(
                ...
                example = {
                    "schema_version": "atomic-microclaim-relation-expanded-v1",
