"""GPU runner v2 for the pre-registered auxiliary exact-span conflict pilot.

The default/CPU commands never load the full checkpoint or initialize CUDA.
V2 forces SDPA and disables reference compilation when loading ModernBERT.
GPU commands are explicit and are never called by the CPU checks.
"""
...
from transformers import (AutoConfig, AutoModelForSequenceClassification, ModernBertConfig,
...
def estimate():
    ...
    save_json(OUT / "GPU_PLAN.json", output)
    print(json.dumps(output, ensure_ascii=False, indent=2))


def cpu_check():
    _complete, arrays = load_data()
    loader_config = AutoConfig.from_pretrained(
        MODEL, local_files_only=True, trust_remote_code=False,
        attn_implementation="sdpa", reference_compile=False)
    assert loader_config._attn_implementation == "sdpa"
    assert loader_config.reference_compile is False
    ...
    result = {
        ...
        "loader_config_attn_implementation": loader_config._attn_implementation,
        "loader_config_reference_compile": loader_config.reference_compile,
        "model_checkpoint_loaded": False, "GPU_used": False,
        "calibration_rows_read": 0, "test_rows_read": 0,
    }
    save_json(OUT / "CPU_CHECK.json", result)
...
def answer_max_from_windows(scores, answer_index, n_answers):
    scores = np.asarray(scores, dtype=np.float64)
    answer_index = np.asarray(answer_index, dtype=np.int64)
    assert len(scores) == len(answer_index)
    result = np.full(n_answers, -np.inf, dtype=np.float64)
    np.maximum.at(result, answer_index, scores)
...
def finalize():
    ...
    answer_y = arrays["held_answer_label"]
    answer_index = arrays["held_window_answer_index"]
    v4_answer = answer_max_from_windows(v4, answer_index, len(answer_y))
    v4_answer_metrics, _ = count(answer_y, v4_answer, choose_threshold(answer_y, v4_answer))
    ...
    for arm in ("candidate", "control"):
        ...
        conflict_answer_metrics, _ = count(
            answer_y, conflict_answer, choose_threshold(answer_y, conflict_answer))
        combined_answer_metrics, _ = count(
            answer_y, combined_answer, choose_threshold(answer_y, combined_answer))
