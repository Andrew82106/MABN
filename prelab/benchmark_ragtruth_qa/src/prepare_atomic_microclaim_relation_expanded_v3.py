...
AUDIT_CODE = ROOT / "research/atomic_microclaim_relation_expanded_v3_audit/audit.py"
...
def protocol():
    return {
        "version": "atomic-microclaim-relation-expanded-v3",
...
def initialize():
    ...
    (OUT / "PLAN.md").write_text(
        "# Expanded atomic-microclaim relation data v3\n\n"
    ...
    with examples_pending.open("w", encoding="utf-8", newline="\n") as example_handle, \
        ...
        for response_index, (partition, source) in enumerate(all_answer_rows):
            ...
            for local_id, (packed, pair_length, premise_length, hypothesis_length) in enumerate(zip(
                ...
                example = {
                    "schema_version": "atomic-microclaim-relation-expanded-v3",
