"""Separate, fixed36-candidate convex comparison for completed PsiloQA transfer.
...
ALPHAS = (0., .2, .4, .6, .8, 1.)
CONFIG = {
    'psiloqa': {'directory': 'full_context_psiloqa_transfer_v1', 'run': 'transfer',
             'script': 'run_psiloqa_transfer.py', 'epochs_key': 'all_QA_epochs',
             'all_epochs': [1, 2, 3], 'eligible_epochs': [1, 2, 3], 'file_prefix': 'qa_epoch_',
             'limits': 'Automatically labelled PsiloQA no-context answers as auxiliary supervision, then original human QA training; auxiliary final checkpoint never enters model selection. Auxiliary labels are not human gold.'}}
...
def protocol(target):
    return {'version': target+'-six-original-controls-convex-v1', 'target': target,
        ...
        'detector': 'One already selected epoch from the complete upstream run; selection belongs to that run, not individual fusion families. PsiloQA permits only its3 QA epochs, never the auxiliary checkpoint.',
        ...
        'limits': [CONFIG[target]['limits'], 'Extra offline semantic detector, not a pure original-generator white-box probe.',
                   ...
                   'Upstream training cost and supervision differ. PsiloQA is an independent comparison, not a cumulative large/NLI/FAVA fusion.'],
...
def static_bindings(target):
    ...
    files = [Path(__file__), Path(q.__file__), Path(original_controls.__file__),
             ...
             prepared/'COMPARISON_PROTOCOL.json',
...
def prepare(target):
    ...
    prior = q.read(prepared/'COMPARISON_PROTOCOL.json')
    assert prior['candidate_count'] == 36 and prior['weights'] == list(ALPHAS)
    assert prior['status'] == 'frozen_future_protocol_not_executed'
    for previous, (family, peer, mode, entry, path) in zip(prior['bases'], original_bases, strict=True):
        assert (previous['family'], previous['peer'], previous['mode']) == (family, peer, mode)
        assert previous['scores_sha256'] == entry['scores_sha256'] == q.sha(path)
        assert previous['scores_path'] == str(path.resolve())
