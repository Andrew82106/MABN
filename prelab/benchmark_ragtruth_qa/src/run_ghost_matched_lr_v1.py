...
def protocol():
    return {'version': 'ghost-matched-local-lr-v1', 'methods': list(METHODS), 'widths': WIDTHS,
            'C': list(CS), 'new_fits': 5, 'reused_old_fits': 20, 'seed': 20260924,
            'old_C_history': list(OLD_CS),
            'fixed_parameter_design': 'Before any new GHOST feature or result, fix new C=1e-4, matching all four previously selected old families. No new C search; conservative same-C signal comparison, not25-fit matched search.',
            ...
            'scaler': 'Separate per family; fit-only base-weighted partial_fit in original16384-row blocks; all transformed inputs float32.',
            ...
            'C_selection': 'No new C selection: one fixed1e-4 candidate per family serves both scales. Preserve original q.selection_key in output only for schema compatibility. Original old5C selection history retained.',
            ...
            'storage': 'Keep5 models/scores/results/scalers; raw four-column matrix. One temporary standardized float32 family memmap, hash recorded, deleted after that family. Original raw matrices stay in place.',
...
def old_controls(meta):
    ...
    for method in old.METHODS:
        ...
        assert best['C'] == CS[0], 'Fixed new C must match the frozen old selected C'
...
def prepare_design():
    ...
    assert OLD_CS == old.CS and CS == (1e-4,)
    ...
    resource = {'reused_old_raw_matrices_GiB': sum(m.nbytes for m in matrices.values())/2**30,
                ...
                'all5_scores_MiB_uncompressed': 5*(NTOTAL+3839)*8/2**20,
                'new_disk_reserve_GiB': 5,
                'old20_observed_CPU_seconds': controls['old20_actual_seconds'],
                'old4_same_C_observed_CPU_seconds': sum(c['selected']['seconds'] for c in controls['controls'].values()),
                'new5_estimated_CPU_minutes': [8, 20],
                'estimate_not_measurement': True, 'threads': 4,
                'note': 'Four high-dimensional fits reuse the previous selected1e-4 budget, plus one4D fit. Timing varies with convergence and CPU contention.'}
...
def require_feature_complete():
    ...
    assert set(complete['files_sha256']) == {'feature_manifest.json', 'signature.json', 'protocol.json', 'CPU_CHECK.json', 'GPU_SMOKE.json'}
    ...
    smoke = q.read(GHOST/'GPU_SMOKE.json')
    assert smoke['status'] == 'passed' and smoke['signature_sha256'] == complete['signature_sha256']
...
def prepare_features():
    ...
    for i, (row, record) in enumerate(zip(inputs, fm['entries'])):
        ...
        for k in ('source_id','group_id','partition','answer_sha256','old_plan_sha256'):
            assert record[k] == row[k]
        assert record['input_ids_sha256'] == q.digest(row['input_ids'])
        assert record['raw_answer_tokens'] == len(row['answer_token_ids'])
...
def fit():
    ...
    q.save(OUT/'fit_started.json', {'preparation_sha256': q.sha(OUT/'feature_preparation_complete.json'),
                                  'protocol_sha256': q.sha(OUT/'protocol.json'), 'new_fit_budget': 5})
    ...
    summary = {'selected': selected, 'all_candidates': candidates, 'old_controls': controls,
               'selected_minus_matched_old_cal_F1': deltas, 'new_fits': 5, 'old_refits': 0,
    ...
    freeze(OUT/'complete.json', {'status': 'complete', 'new_fits': 5, 'old_refits': 0,
                               'files_sha256': {n:q.sha(OUT/n) for n in files}, 'test_opened': False, 'GPU_used': False})
    print('GHOST_MATCHED_COMPLETE5',flush=True)
