...
def prepare():
    ...
    # Six fixed cohort-edge anchors plus both original non-crossing boundaries;
    # chosen only from input token geometry, never from labels or scores.
    anchors = []
    anchor_indices = sorted({0, 633, 634, 3679, 3680, 3838} |
        {ix for ix, r in enumerate(rows) if r['original']['response_token_offsets_raw'][0][0] >= 0})
    assert len(anchor_indices) == 8
    for ix in anchor_indices:
        row = rows[ix]
        target = sources[row['source_id']]
    ...
    assert counts['boundary_crossing_answers'] == sum(
        r['original']['response_token_offsets_raw'][0][0] < 0 for r in rows)
