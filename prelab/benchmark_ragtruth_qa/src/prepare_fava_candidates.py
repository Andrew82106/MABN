...
def protocol():
    return {'version':'fava-factual-only-silver-candidate-data-v1',
        ...
        'material_match':'Only evidence text. Existing Unicode-word norm re.findall(r"\\w+",text.casefold()); match any20 consecutive normalized words, separately within each evidence part. Also quarantine exact raw evidence-part SHA matches, even short parts. QA questions and schema boilerplate are excluded using unchanged existing evidence_parts.',
        'FAVA_groups':'Before filtering, connect all7483 fixed candidates sharing any exact original reference-block SHA. Any component touching blocked RT material is wholly quarantined. Distractor blocks can create links; group IDs are conservative material components, not independent entities/events/articles.',
...
def run():
    ...
    uf=Union(records)
    for members in ref_members.values():
    ...
    for sid,source in sources.items():
        for part_index,part in enumerate(rules.evidence_parts(source)):
            h=digest(part);exact.setdefault(h,[]).append((sid,part_index))
    ...
    save_rows('shared_reference_block_index.jsonl',[{'reference_text_sha256':h,
        ...
        for h,members in sorted(ref_members.items()) if len({i for i,o in members})>1])
    ...
    artifacts=['candidate_fit.jsonl','fixed_candidate_index.jsonl','duplicate_index.jsonl','quarantined_candidate_index.jsonl',
        ...
        'rt_source_identity_index.jsonl','rt_material_group_index.jsonl','SOURCE_ISOLATION_REPORT.json','DATA_PROTOCOL.json']
    assert frozen==check()
