"""Explicit CPU preparation for matched local silver BCE / BCE+ranking transfer.
...
ROOT=q.ROOT;OUT=ROOT/'results/local_pair_transfer_v1'
DATA=ROOT/'auxiliary_fava_local_pairs_v1';TOKENS=DATA/'tokenization_v1'
VARIANTS={'paired_bce':0.,'paired_rank':1.}
BATCH=8;NQA=3680;QA_EPOCHS=3;PAIR_ORDER_SEED=20261007;MARGIN=1.
...
def protocol(cohort):
    ...
    return {'version':'local-pair-silver-transfer-v1','variants':VARIANTS,'margin':MARGIN,
        'initialization':'Fresh generic ModernBERT-base via original v2.load_model, same pretrained revision and seed20261005 for each variant. No QA-fitted initialization.',
        ...
        'loss':'Exactly frozen local_repair_pair_loss: half original local BCE(y=1)+half repaired local BCE(y=0); plus lambda*softplus(1-mean_bad+mean_repaired). No rescaling of the sum, no new class factors. Silver editing roles are not certified truth.',
        'pair_weights':'Final eligible pairs only, equal material groups -> equal original answers within group -> equal eligible pairs per original answer. Both versions are one unit. Sum base weights1.',
        ...
        'matched_compute':'Same pair IDs, order, target positions, base weights, two forwards, local BCE and rank calculation in both modes; only multiplication weight0 versus1 changes. Same model seed, optimizer, QA inputs/weights/orders and budgets.',
        ...
        'QA':{'fit_answers':3680,'fit_groups':615,'calibration_answers':159,'epochs':3,'mass':560300,
              ...
              'selection':'Only after all3 QA epochs complete, one epoch per variant by min(windowF1,answerF1),windowF1,windowprecision,earlier epoch. Each epoch uses original separate cal thresholds and precision/higher-cutoff ties. No auxiliary cal evaluation or checkpoint selection.'},
        'artifacts':'Each variant independently keeps auxiliary_final and QA1/2/3 model+optimizer+CPU/CUDA RNG; all QA raw-token predictions, geometry scores, thresholds, logs, logical tokens/time/peak memory. No automatic resume or overwrite.',
        ...
        'scope':'CPU preparation only until separate explicit GPU commands. No additional model/grid/score-combination branch.',
...
def prepare():
    ...
    index=[p for p in allpairs if p['eligible_for_future_local_pair_training']]
    assert len(index)==tm['eligible_pairs'] and all(not p['ineligibility_reasons'] for p in index)
    weights,tree=pair_base_weights(index)
    for p,w in zip(index,weights):assert p['eligible_pair_base_weight']['value']==w
    n=len(index);ng=len(tree);na=sum(len(x) for x in tree.values())
    assert ng==tm['eligible_material_groups'] and na==tm['eligible_original_answers']
    ...
    cohort={'candidate_pairs':10040,'eligible_pairs':n,'excluded_from_training_but_retained':10040-n,
    ...
    resources={'pair_comparison_same_inputs_and_forward_count':True,'QA_three_files_byte_identical':True,
        ...
        'longest_complete_pair':cohort['longest_pair_lengths'],'checkpoints_per_variant':4,'variants':2,
        'existing_same_model_QA_checkpoint_bytes':oldsize,'estimated_all8_checkpoint_bytes':8*oldsize if oldsize else None,
        'free_disk_bytes_at_prepare':shutil.disk_usage(OUT).free,
        'GPU_hours_per_variant_planning':[2.,5.],'GPU_estimate_not_measured':True,
        ...
        'time_limit':'Order-of-magnitude budget; input length and two-graph/backward/checkpoint costs require actual smoke/runtime. No parameter choice based on scores.',
        'gradient_strength_limit':'lambda1 increases objective gradient magnitude as well as shape; fixed-budget comparison is not an isolated scale-invariant proof.',
    ...
    assert not torch.cuda.is_initialized();print('LOCAL_PAIR_TRANSFER_PREPARED',json.dumps(cohort),flush=True)
...
def gpu_smoke():
    ...
    try:
        ...
        # Exactly production paired two-graph path, with maximum ranking branch;
        # no QA labels/scores, no checkpoint/parameter selection in this gate.
        run=train_pair_epoch(model,opt,[pair],np.array([0]),np.array([1.]),'cuda','paired_rank',progress=False)
        ...
        q.save(OUT/'GPU_SELFCHECK.json',{'passed':True,'pair_id':pair['pair_id'],'input_tokens':p['cohort']['longest_pair_lengths'],
            'production_two_forward_graphs_backward':True,'ranking_weight':1.,'margin':1.,'no_QA_gold_or_calibration_used':True,
...
def cpu_test():
    ...
    assert reports['paired_bce']['pair_order_sha256']==reports['paired_rank']['pair_order_sha256']
    assert reports['paired_bce']['forward_calls']==reports['paired_rank']['forward_calls']==22
    assert reports['paired_bce']['logical_input_tokens']==reports['paired_rank']['logical_input_tokens']
    ...
    q.save(OUT/'CPU_SELFCHECK.json',{'passed':True,'production_pair_loop_TinyModernBERT':True,'same_initial_states':True,
        ...
        'two_variants_same_data_order_forward_count':True,'variants':reports,'preparation_sha256':q.sha(OUT/'preparation_complete.json'),
        'real_dataset_training':False,'pretrained_model_loaded':False,'GPU_used':False,'official_test_opened':False})
    print('LOCAL_PAIR_CPU_TEST_PASSED_BOTH_VARIANTS',flush=True)
...
def main():
    ...
    with threadpool_limits(limits=4):
        try:
            ...
            elif args.stage=='check':check_prepared();print('LOCAL_PAIR_PREPARED_CHECK_PASSED',flush=True)
