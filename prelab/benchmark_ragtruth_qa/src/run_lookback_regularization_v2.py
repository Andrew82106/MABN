...
OUT=ROOT/'results/lookback_regularization_v2';METHODS=ctrl.METHODS;NEW_C=(1e-5,1e-4);GRID=NEW_C+(.001,.01,.1)
def now():return datetime.now(timezone.utc).isoformat()

def protocol():
    return {'version':'qa-lookback-common-stronger-regularization-v2-snapshot-fixed','methods':list(METHODS),'C':list(GRID),'added_C':list(NEW_C),
...
def verify_sources():
    ...
    for method in METHODS:
        if method!=ctrl.ANCHOR:
            p=OLD/'matrices'/(method+'_fit_standardized.npy');files[str(p.resolve())]=q.sha(p)
...
def run():
    ...
    for method in METHODS:
        ...
        if method!=ctrl.ANCHOR:
            ...
            assert snapshot['files_sha256'][str(zp.resolve())]==q.sha(zp)
        family=[]
        for c in GRID:
    ...
    assert q.read(OUT/'source_snapshot.json')==snapshot
    assert q.read(OUT/'started.json')['source_snapshot_sha256']==q.sha(OUT/'source_snapshot.json')
