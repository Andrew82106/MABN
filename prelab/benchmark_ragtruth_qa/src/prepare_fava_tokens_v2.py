...
DATA = q.ROOT / 'auxiliary_fava_v2'
OUT = DATA / 'tokenization_v2'
MODEL = q.ROOT.parent / 'models/ModernBERT-base'
FACT_TYPES = {'entity', 'relation', 'invented', 'contradictory'}


def protocol():
    return {'version': 'fava-silver-token-preparation-v2-NFC-aware',
        ...
        'mapping': 'Keep old map exactly whenever all original nonspace characters have owners. Only for proven single-character NFC compositions, auxiliary-only helper assigns missing consumed combining marks to actual encoder owners of the starter, preserving original-character mass. Other gaps are retained in exception ledger and block training. Main QA mapping, labels, windows and splits remain unchanged.',
...
def prepare():
    ...
    q.save(OUT/'token_design_freeze.json', {'protocol_sha256': q.sha(OUT/'token_protocol.json'),
        'source_sha256': {str(p.resolve()): q.sha(p) for p in [Path(__file__), Path(replay.__file__), Path(gold.__file__), Path(mapping.__file__), Path(nfc_mapping.__file__), MODEL/'tokenizer.json']},
...
def run():
    ...
    manifest = q.read(DATA/'manifest.json')
    source_path = DATA/'candidate_fit.jsonl'
    # The independent data stage must finish before this command can be invoked.
    complete = q.read(DATA/'complete.json')
    assert q.sha(DATA/'manifest.json') == complete['manifest_sha256']
    ...
    counts = Counter(); lengths = []; exceptions = []; repairs = []; started = time.perf_counter()
    with (OUT/'token_inputs.jsonl').open('w', encoding='utf-8') as stream:
        for i, row in enumerate(rows):
            ...
            try:
                ...
                exceptions.append({'response_id': row['response_id'], 'stage': 'NFC_character_mapping', 'error': str(exc), 'original_text_and_gold_changed': False})
                continue
            if proof['repaired_character_count']:
                repairs.append({'response_id': row['response_id'], **proof})
                counts.update(nfc_repaired_answers=1, nfc_repaired_characters=proof['repaired_character_count'])
            n = len(offsets)
    ...
    q.savel(OUT/'nfc_repairs.jsonl', repairs)
    report = {'status': 'prepared_not_trained' if not exceptions else 'review_required_no_silent_filtering',
        'expected_answers': len(rows), 'answers': counts['answers'], 'stats': dict(counts), 'median_input_tokens': float(np.median(lengths)),
        'exceptions': exceptions, 'no_truncation': True, 'GPU_used': False, 'trained': False,
        'manifest_sha256': q.sha(DATA/'manifest.json'), 'candidate_sha256': q.sha(source_path),
        'nfc_repairs_sha256': q.sha(OUT/'nfc_repairs.jsonl'), 'token_design_sha256': q.sha(OUT/'token_design_freeze.json'), 'token_inputs_sha256': q.sha(OUT/'token_inputs.jsonl'),
