...
ROOT = HERE.parent
OUT = ROOT / "results/atomic_microclaim_relation_expanded_v4"
...
AUDIT_CODE = ROOT / "research/atomic_microclaim_relation_expanded_v4_audit/audit.py"
...
def protocol():
    return {
        "version": "atomic-microclaim-relation-expanded-v4",
...
def eligible_windows(lexical, risk):
    eligible, positive = [], []
    for start in range(max(1, len(lexical) - WINDOW_K + 1)):
...
def self_test():
    ...
    assert eligible_windows([1], [0]) == ([0], [])
    return {"status": "passed", "token_ownership": True, "window_k": WINDOW_K,
            "short_answer_window": True, "model_loaded": False, "GPU_used": False,
        ...
        for response_index, (partition, source) in enumerate(all_answer_rows):
            ...
            for local_id, (packed, pair_length, premise_length, hypothesis_length) in enumerate(zip(
                ...
                mapped_windows = sorted({window_start for token_index in info["lexical_bpes"]
                                         ...
                                         if window_start <= token_index < min(len(lexical), window_start + WINDOW_K)})
                ...
                example = {
                    "schema_version": "atomic-microclaim-relation-expanded-v4",
            ...
            for start in eligible:
                local_claims = sorted({claim_id for token_index in range(start, min(len(owners), start + WINDOW_K))
