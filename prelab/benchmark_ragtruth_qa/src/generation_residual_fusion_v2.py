"""Independent v2 residual head retaining absolute LB mean and dispersion."""
...
class GenerationResidualFusion(nn.Module):
    ...
    def components(
        ...
        # Upstream verified LB lies in [0,1]. Both summaries are in [0,1]:
        # population std <= .5. Keep these outside the per-token LayerNorm;
        # no fitted scaler, clipping or additional normalization is applied.
        lookback_stats = torch.stack((lookback.mean(dim=-1),
                                      2 * lookback.std(dim=-1, unbiased=False)), dim=-1)
        fused = torch.cat((semantic, generation, baseline_logit.unsqueeze(-1), lookback_stats), dim=-1)
        gate = torch.sigmoid(self.gate(fused)).squeeze(-1)
        raw_residual = self.residual(fused).squeeze(-1)
        correction = gate * raw_residual
        return {'semantic': semantic, 'generation': generation, 'lookback_stats': lookback_stats, 'gate': gate,
