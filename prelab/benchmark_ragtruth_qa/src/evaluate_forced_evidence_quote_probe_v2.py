...
V1_EVALUATOR_PATH = HERE / "evaluate_forced_evidence_quote_probe_v1.py"
...
# V2 changes only GPU numerical provenance.  Keeping the V1 salt preserves the
# already frozen group folds byte-for-byte.
...
SCIENTIFIC_FUNCTIONS = (
    "fold_for", "load_real_gold_geometry", "answer_normalized_base_weights",
    "fit_predict", "project_to_windows", "f1_precision",
    "choose_threshold", "fixed_ap", "aggregate_answers", "run_nested_cv",
    "make_conditions", "type_diagnostics", "compute_advance_gate",
    "serializable_result", "pairwise_fold_deltas",
)
...
def scientific_function_hashes(path: Path) -> dict[str, str]:
    """Hash scoring-function ASTs so V2 can prove its V1 scoring parity."""
    tree = ast.parse(Path(path).read_text(encoding="utf-8"))
    functions = {node.name: node for node in tree.body
                 if isinstance(node, (ast.FunctionDef, ast.AsyncFunctionDef))}
    missing = set(SCIENTIFIC_FUNCTIONS) - set(functions)
    if missing:
        raise ProtocolFailure(f"scientific function set incomplete: {sorted(missing)}")
    return {
        name: digest_text(ast.dump(functions[name], include_attributes=False))
        for name in SCIENTIFIC_FUNCTIONS
    }


def assert_v1_scoring_parity() -> dict[str, str]:
    current = scientific_function_hashes(Path(__file__))
    reference = scientific_function_hashes(V1_EVALUATOR_PATH)
    if current != reference:
        changed = sorted(name for name in current
                         if current[name] != reference[name])
        raise ProtocolFailure(f"V1 scientific scoring code changed in V2: {changed}")
    if SALT != "forced_evidence_quote_probe_v1":
        raise ProtocolFailure("V1 fold salt changed")
    return current
...
def load_clean_rows() -> list[dict]:
    ...
    if sha(NUMERICAL_PROTOCOL_PATH) != EXPECTED_NUMERICAL_PROTOCOL_SHA:
        raise ProtocolFailure("V2 numerical protocol changed")
    if sha(GPU_RUNNER_PATH) != EXPECTED_RUNNER_SHA:
        raise ProtocolFailure("V2 GPU runner changed")
...
class FrozenFeatureReceipt:
    ...
    row_keys_sha256: str
    runner_sha256: str
    numerical_protocol_sha256: str
    extraction_complete_sha256: str
    full_batch_qa_sha256: str
...
def load_frozen_features(feature_dir: Path = FEATURE_DIR) -> tuple[FrozenFeatureReceipt, list[dict]]:
    ...
    expected_freeze_keys = {
        ...
        "GPU_extraction_complete_sha256", "full_batch_cache_replay_QA_sha256", "rows",
    ...
    if freeze["schema_version"] != FEATURE_SCHEMA_VERSION:
        raise ProtocolFailure("feature freeze version mismatch")
    ...
    if (freeze["label_free_inputs_sha256"], freeze["protocol_sha256"],
            freeze["plan_sha256"], freeze["numerical_protocol_sha256"],
            freeze["GPU_runner_sha256"], int(freeze["rows"])) != (
            EXPECTED_INPUT_SHA, EXPECTED_PROTOCOL_SHA, EXPECTED_PLAN_SHA,
            EXPECTED_NUMERICAL_PROTOCOL_SHA, EXPECTED_RUNNER_SHA, EXPECTED_CLAIMS):
    ...
    for key in ("GPU_extraction_complete_sha256",
        ...
        if not re.fullmatch(r"[0-9a-f]{64}", str(freeze[key])):
            raise ProtocolFailure(f"feature freeze {key} is not SHA-256")
    ...
    if manifest["schema_version"] != FEATURE_SCHEMA_VERSION:
        raise ProtocolFailure("feature manifest version mismatch")
    ...
    if (int(manifest["rows"]), manifest["row_keys_sha256"],
            ...
            manifest["plan_sha256"], manifest["numerical_protocol_sha256"],
            manifest["GPU_runner_sha256"],
            manifest["GPU_extraction_complete_sha256"],
            manifest["full_batch_cache_replay_QA_sha256"]) != (
            EXPECTED_CLAIMS, keys_digest, EXPECTED_INPUT_SHA,
            EXPECTED_PROTOCOL_SHA, EXPECTED_PLAN_SHA,
            EXPECTED_NUMERICAL_PROTOCOL_SHA, EXPECTED_RUNNER_SHA,
            freeze["GPU_extraction_complete_sha256"],
            freeze["full_batch_cache_replay_QA_sha256"]):
    ...
    receipt = FrozenFeatureReceipt(
        ...
        row_keys_sha256=keys_digest,
        runner_sha256=manifest["GPU_runner_sha256"],
        numerical_protocol_sha256=manifest["numerical_protocol_sha256"],
        extraction_complete_sha256=manifest["GPU_extraction_complete_sha256"],
        full_batch_qa_sha256=manifest["full_batch_cache_replay_QA_sha256"],
        arrays=arrays,
    )
    return receipt, rows
...
def evaluate_real() -> None:
    ...
    scoring_hashes = assert_v1_scoring_parity()
    ...
    output = {
        ...
        "feature_receipt": {
            ...
            "GPU_runner_sha256": receipt.runner_sha256,
            "numerical_protocol_sha256": receipt.numerical_protocol_sha256,
            "GPU_extraction_complete_sha256": receipt.extraction_complete_sha256,
            "full_batch_cache_replay_QA_sha256": receipt.full_batch_qa_sha256,
        },
        "gold_source_hashes": geometry.source_hashes,
        "evaluator_sha256": sha(Path(__file__)),
        "V1_scientific_function_AST_sha256": scoring_hashes,
        "scientific_training_and_evaluation_protocol_unchanged_from_V1": True,
...
def synthetic_selftest() -> None:
    ...
    report = {
        ...
        "GPU_runner_sha256": sha(GPU_RUNNER_PATH),
        ...
        "checks": {
            ...
            "V1_scientific_function_AST_parity": True,
            "V1_fold_salt_preserved": True,
        },
        "V1_scientific_function_AST_sha256": scoring_hashes,
