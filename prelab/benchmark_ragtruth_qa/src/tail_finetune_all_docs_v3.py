...
ROOT=qa.ROOT; EXP=ROOT/'fit_expansion'; OUT=ROOT/'results/minicheck_tail_all_docs_v3'
OLD=ROOT/'semantic_baseline/cuda_variant'; NEW=EXP/'minicheck'; BACK=EXP/'minicheck_backfill_v2'; EXTRA=EXP/'minicheck_unselected_v2'
...
def protocol():
    p=old.protocol();p.update({
        'version':'qa-minicheck-all-document-token-v3-original-batch-geometry',
        ...
        'numeric_smoke':{'indices':'first/last fit, first/last cal, longest total valid input answer, first multi-document answer, known numeric batch-regression answer13953 (deduplicated; no label/performance selection)',
        ...
        'cache_path_binding':'Physical original793 backfill directory may be specified before prepare; it cannot change expected original plans/answers/doc choices/IDs/coordinates/numeric tolerances. Actual source directories and every source hash are frozen in prepare and must match for all later commands.',
        'sequence_microbatch':4,
        'gradient_checkpointing':'Non-reentrant checkpoint of both tail layers, original per-answer all-doc batches of4 in claim-major/document-major order; original row, padded length, and final partial batch size retained. Front22 cached states detached. No per-document rebatching.',
        'padding_repair':'V2 had no real training and is retained. Microbatch2->original4 corrects numeric replay geometry, not performance tuning; all3epochs, optimizer, head initialization, loss and answer orders unchanged. Selected and complement front/final states must originate from same original full Cartesian batch shape.',
        'padding_values':'Only original valid cached positions exist. Pad cached hidden positions with0 and retain original attention mask0; masked keys cannot support valid queries. Actual same-doc final-state strict numeric smoke is required; CPU equivalence is not a claim of GPU identity.',
        ...
        'resource_estimate':'25,193,473 trainable parameters; FP32 weights+grad+Adam about403MB before activations. Original batch4 checkpoint is required;8GB feasibility is measured by GPU smoke and not assumed.'})
    return p
...
def fixed_source_paths():
    return [Path(__file__),Path(old.__file__),Path(qa.__file__),*PLANS,
        ...
        ROOT/'src/tail_finetune_all_docs.py',PREVIOUS/'protocol_freeze.json',PREVIOUS/'protocol.json',PREVIOUS/'training_weights.npz',PREVIOUS/'answer_orders.npy']
...
def freeze():
    ...
    with np.load(PREVIOUS/'training_weights.npz',allow_pickle=False) as prev:
        assert set(prev.files)==set(weights)
        for key,value in weights.items():assert np.array_equal(prev[key],np.asarray(value)),key
    assert np.array_equal(np.load(PREVIOUS/'answer_orders.npy'),np.load(OUT/'answer_orders.npy'))
    pp=qa.read(PREVIOUS/'protocol.json')
    for key in ('epochs','seed','effective_answer_batch','optimizer','selection','weights','token_loss','aggregation'):
        assert protocol()[key]==pp[key],key
    qa.save(OUT/'FAIRNESS_CHECK.json',{'passed':True,'previous_v2_weights_all_arrays_exact':True,'three_answer_orders_exact':True,
        'epochs_optimizer_loss_aggregation_selection_identical':True,'only_microbatch_numeric_geometry_changed':True,
        'original_sequence_microbatch':2,'new_original_sequence_batch':4,'same_updates_per_epoch':460,'same_updates_each_condition':1380,
    ...
    paths=fixed_source_paths()+[OUT/x for x in ('protocol.json','training_weights.npz','answer_orders.npy','dataset_index.json','CPU_ALLDOC_TEST.json',
        'REAL_SELECTED_INTERFACE_CHECK.json','EVALUATION_GEOMETRY_CHECK.json','FAIRNESS_CHECK.json')]
...
class AllDocDataset:
    ...
    def load(self,i,verify_hash=False,need_hidden=True):
        ...
        for path,record in self.encoder[rid]:
            ...
            assert z['batch_padding_length'].shape==(len(off)-1,)
            if path.parent.parent in (BACK,EXTRA):
                assert all(z[k].shape==(len(off)-1,) for k in ('original_batch_index','original_batch_row','original_batch_size','original_batch_padding_length'))
            for j,(ci,di) in enumerate(zip(z['claim_index'],z['document_index'])):
                ...
                flat=key[0]*D+key[1];batch_start=4*(flat//4);batch_end=min(batch_start+4,C*D)
                expected_padding=max(p['pair_lengths'][batch_start:batch_end])
                assert int(z['batch_padding_length'][j])==expected_padding,(rid,key,'Cache did not use original all-doc padding')
                if path.parent.parent in (BACK,EXTRA):
                    assert (int(z['original_batch_index'][j]),int(z['original_batch_row'][j]),int(z['original_batch_size'][j]),int(z['original_batch_padding_length'][j]))==(flat//4,flat%4,batch_end-batch_start,expected_padding)
                pair.update(original_batch_start=batch_start,original_batch_row=flat-batch_start,
                    original_batch_size=batch_end-batch_start,batch_padding_length=expected_padding)
        ...
        flat_pairs=[enc[(ci,di)] for ci in range(C) for di in range(D)];batches=[]
        for first in range(0,len(flat_pairs),4):
            block=flat_pairs[first:first+4];size=len(block);padding=max(len(e['input_ids']) for e in block)
            for row,e in enumerate(block):
                assert e['original_batch_start']==first and e['original_batch_row']==row and e['original_batch_size']==size and e['batch_padding_length']==padding
            batches.append({'pairs':block,'batch_start':first,'batch_size':size,'padding_length':padding})
        return {'response_id':rid,'docs':docs,'batches':batches,'n_raw':t['token_count'],'nonspace':nonspace}
...
def pack_original_batch(batch,hidden_size,device):
    pairs=batch['pairs'];assert len(pairs)==batch['batch_size'] and 1<=len(pairs)<=4
    length=batch['padding_length'];assert length==max(len(p['input_ids']) for p in pairs)
    h=torch.zeros(len(pairs),length,hidden_size,dtype=torch.float32,device=device)
    mask=torch.zeros(len(pairs),length,dtype=torch.long,device=device)
    for row,pair in enumerate(pairs):
        assert pair['original_batch_row']==row and pair['original_batch_size']==len(pairs) and pair['batch_padding_length']==length
        n=len(pair['input_ids']);h[row,:n]=torch.as_tensor(pair['hidden22'],device=device);mask[row,:n]=1
...
def tail_logits(model,bundle,device,checkpointed=False):
    ...
    for batch in bundle['batches']:
        h,mask=pack_original_batch(batch,model.config.hidden_size,device);z=model(h,mask,checkpointed=checkpointed)
        for row,pair in enumerate(batch['pairs']):
            key=(pair['claim_index'],pair['document_index']);assert key not in by_pair
            by_pair[key]=z[row,:len(pair['input_ids'])]
    values=[]
    for doc in bundle['docs']:
        parts=[by_pair[(pair['claim_index'],pair['document_index'])] for pair in doc['pairs']]
...
def prepare():
    ...
    for i,a in enumerate(answers):
        ...
        checks.append({'response_id':a['response_id'],'documents':len(ranges),'claims':len(ds.plans[a['response_id']]['claims']),
            ...
            'original_batches':[{'batch_start':g['batch_start'],'batch_size':g['batch_size'],'padding_length':g['padding_length']} for g in b['batches']]})
    ...
    qa.save(OUT/'source_snapshot.json',{'files_sha256':source,'protocol_freeze_sha256':qa.sha(OUT/'protocol_freeze.json'),
        'backfill_directory':str(BACK.resolve()),'unselected_directory':str(EXTRA.resolve()),'test_opened':False})
...
def check_prepared():
    ...
    assert snap['unselected_directory']==str(EXTRA.resolve())
...
def gpu_smoke():
    ...
    indices=sorted({0,NFIT-1,NFIT,NDEV-1,max(range(NDEV),key=lambda i:sum(ds.plans[answers[i]['response_id']]['pair_lengths'])),
        ...
        next(i for i,a in enumerate(answers) if a['response_id']=='13953')})
    ...
    with torch.no_grad():
        for i in indices:
            ...
            for batch in bundle['batches']:
                h,mask=pack_original_batch(batch,model.config.hidden_size,'cuda')
                first=model.tail_hidden(h,mask);second=model.tail_hidden(h,mask)
                for row,pair in enumerate(batch['pairs']):
                    one=first[row];two=second[row];ix=pair['claim_input_index']
            ...
            records.append({'index':i,'response_id':answers[i]['response_id'],'claim_positions_compared':compared,
                ...
                'original_batches':[{'batch_start':g['batch_start'],'batch_size':g['batch_size'],'padding_length':g['padding_length']} for g in bundle['batches']]})
...
def tiny_test():
    ...
    raw2=[[0,2],[2,5]];bundle={'n_raw':2,'docs':[]};allpairs={}
    for di in range(2):
        pairs=[];ss=[];ee=[];reference=[]
        for ci in range(2):
            k=ci*2+di;n=int(mask[k].sum());s=np.full(n,-1,np.int64);e=s.copy()
            if ci==0:s[2:6]=[0,1,1,2];e[2:6]=[1,2,2,3]
            else:s[2]=4;e[2]=5
            pair={'input_ids':ids[k,:n].numpy(),'hidden22':hidden[k,:n].numpy(),'claim_index':ci,'document_index':di,
                'original_batch_row':k,'original_batch_size':4,'batch_padding_length':9};pairs.append(pair);allpairs[(ci,di)]=pair
    ...
    bundle['batches']=[{'pairs':[allpairs[(ci,di)] for ci in range(2) for di in range(2)],'batch_size':4,'padding_length':9}]
    packed,pmask=pack_original_batch(bundle['batches'][0],32,'cpu')
    assert torch.equal(pmask,mask) and torch.equal(packed[mask.bool()],hidden[mask.bool()])
    partial=copy.deepcopy(bundle['batches'][0]);partial['pairs']=partial['pairs'][:3];partial['batch_size']=3
    for row,pair in enumerate(partial['pairs']):pair['original_batch_row']=row;pair['original_batch_size']=3
    ph,pk=pack_original_batch(partial,32,'cpu');assert ph.shape==(3,9,32) and torch.equal(pk,mask[:3])
    ...
    qa.save(OUT/'CPU_ALLDOC_TEST.json',{'passed':True,'tiny24layer_replay_max_abs':error,'tail_gradient_norms':norms,
        ...
        'front_parameters_unchanged':True,'original_window_geometry':True,'original_batch4_row_padding_exact':True,'partial_batch_size_preserved':True,
        'GPU_used':False,'real_training_performed':False})
