"""Executable GPU feature runner for forced-evidence quote probe v1.
...
replay.  P2 tokenizes the complete prompt+mechanical quote once and uses a
teacher-forced no-cache forward.  Every formal claim is committed atomically.
"""
...
VERSION = "forced-evidence-quote-probe-gpu-runner-v1"
...
def load_inputs():
    ...
    assert sha(INPUT_PATH) == "ae6bf145a0e48ff310cdde5f54457eec7e8986c9b012d52cc463bb9e357e2777"
...
def _assert_cache_replay_oracle(cached, replay, generated_ids, plan):
    oracle = plan["whitebox_execution"]["smoke_oracle"]
    assert np.array_equal(replay["argmax_token_ids"], generated_ids)
    errors = {}
    mapping = {
        "selected_logprob": "cache_vs_replay_selected_logprob",
        "vocab_entropy": "cache_vs_replay_vocab_entropy",
        "signed_margin": "cache_vs_replay_signed_margin",
    }
    for key, tolerance_name in mapping.items():
        tolerance = oracle[tolerance_name]
        assert np.allclose(cached[key], replay[key],
                           rtol=tolerance["rtol"], atol=tolerance["atol"]), key
        errors[key] = float(np.max(np.abs(cached[key] - replay[key])))
    return errors


def extract_p3(model, tokenizer, plan, row, pca, candidates):
    ...
    del final
    oracle_errors = _assert_cache_replay_oracle(
        cached, replay, cached["generated_token_ids"], plan)
    ...
    metadata = {
        ...
        "input_tokens": len(full_ids),
        "parse": parsed,
        "cache_replay_max_abs_error": oracle_errors,
    }
    return arrays, metadata, cached


def extract_claim(model, tokenizer, plan, row, pca):
...
def validate_arrays(arrays: dict):
    ...
    assert np.array_equal(arrays["p3_replay_argmax_token_ids"], generated)
    ...
    assert np.all(arrays["p3_generated_selected_logprob"] <= 1e-5)
    assert np.all(arrays["p3_generated_vocab_entropy"] >= 0)
    assert np.all(arrays["p3_generated_signed_margin"] >= -1e-5)
    return True
...
def cpu_selfcheck():
    ...
    expected_mechanical = read_json(OUT / "PREPARATION.json")["digests"][
    ...
    with tempfile.TemporaryDirectory(prefix="forced_quote_gpu_runner_cpu_") as directory:
        save_record(0, first, arrays, {"synthetic_CPU_only": True},
                    directory=directory)
...
def _repeat_p3_check(first_arrays, second_arrays, plan):
    assert np.array_equal(first_arrays["p3_generated_token_ids"],
                          second_arrays["p3_generated_token_ids"])
    tolerance = plan["whitebox_execution"]["smoke_oracle"][
        "repeated_replay_hidden64_and_attention"]
    checked = ["p3_hidden64", "p3_attention_summary256",
               "p3_ratio_token_mean", "p3_ratio_token_min",
               "p3_ratio_token_max", "p3_passage_mass_mean",
               "p3_claim_mass_mean"]
    errors = {}
    for key in checked:
        assert first_arrays[key].shape == second_arrays[key].shape
        assert np.allclose(first_arrays[key], second_arrays[key],
                           rtol=tolerance["rtol"], atol=tolerance["atol"]), key
        errors[key] = float(np.max(np.abs(
            first_arrays[key].astype(np.float32) -
            second_arrays[key].astype(np.float32)))) if first_arrays[key].size else 0.0
    return errors
        ...
        try:
            ...
            for index in chosen:
                ...
                if index in repeated_indices:
                    mechanical, candidates = sentence_candidates(row)
                    del mechanical
                    repeated, _repeat_meta, _cached = extract_p3(
                        model, tokenizer, plan, row, pca, candidates)
                    repeat_errors = _repeat_p3_check(arrays, repeated, plan)
                torch.cuda.synchronize()
                report = {
                    "record_index": index,
                    "microclaim_id": row["microclaim_id"],
                    ...
                    "cache_replay_max_abs_error": metadata["P3"][
                        "cache_replay_max_abs_error"],
            ...
            smoke = {
                ...
                "shortest_and_longest_replay_hidden_attention_within_tolerance": True,
                "all_eight_cache_vs_replay_oracle_passed": True,
...
def extract():
    ...
    assert pending, "All records exist but completion marker is absent; audit first"
    started = time.perf_counter()
    model = None
    ...
    records = []
    for index, row in enumerate(rows):
        commit = read_record(index, row, load_arrays=False)
        assert commit is not None
        _npz, _meta, commit_path = record_paths(index)
        records.append({
            "record_index": index,
            "response_id": row["response_id"],
            "microclaim_id": row["microclaim_id"],
