...
def main():
    ...
    with threadpool_limits(limits=4):
        for fold, groups in enumerate(folds):
            ...
            assert f['PCA'] is None
            model = f['model']
            control_path = run.OLD / f'fold_{fold}_frozen.pkl'
            assert f['PCA32_control_model_sha256'] == q.sha(control_path)
            old = pickle.loads(control_path.read_bytes())['model']
            assert np.array_equal(ix, model['fit_ix'])
            assert np.array_equal(model['fit_y'], wy[ix])
            assert model['fit_keys'] == [windows[i]['window_key'] for i in ix]
            for name in ('fit_ix','fit_y','base_weights','loss_weights','class_factors'):
                assert np.array_equal(model[name],old[name])
            for name in ('fit_keys','fit_groups','C','loss_mass'):
                assert model[name] == old[name]
            assert not set(model['fit_groups']) & (set(groups['calibration_groups']) | set(groups['evaluation_groups']))
            design = np.column_stack((hidden,logit)).astype(np.float32)
            assert design.shape == (12222,769)
            # StandardScaler.partial_fit casts sample_weight to X.dtype first.
            scaler_weights = model['base_weights'].astype(design.dtype)
            sm = np.average(design[ix].astype(np.float64), axis=0, weights=scaler_weights)
            sv = np.average((design[ix]-sm)**2, axis=0, weights=scaler_weights)
            ...
            assert model['C'] == .01 and model['width'] == 769 and int(max(model['model'].n_iter_)) < 2000
            ...
            reports.append({'fold': fold, 'assigned_fit_groups': len(groups['fit_groups']), 'active_loss_groups': len(mass),
                ...
                'representation_width': 769, 'PCA_control_width': old['width'],
    ...
    q.save(OUT / 'AUDIT.json', {'passed': True, 'folds': reports,
        ...
        'fit_only_scaler_labels_weights_and_model_replay_checked': True,
    ...
    print('R29_INDEPENDENT_FROZEN_REPLAY_AUDIT_PASSED', flush=True)
